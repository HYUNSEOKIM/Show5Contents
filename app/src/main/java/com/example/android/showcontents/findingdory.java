...
public class findingdory extends AppCompatActivity {
    ...
    private String createMessage(boolean title, boolean genre, boolean story, boolean release, boolean production){
        ...
        if (story){
            Message += "\nStory : Dory (Ellen DeGeneres) is a wide-eyed, blue tang fish who suffers from memory loss every 10 seconds or so." +
                    "\nThe one thing she can remember is that she somehow became separated from her parents as a child." +
                    ...
                    "\nDory now knows that her family reunion will only happen if she can save mom and dad from captivity.\n";
        }

        if (release){
            Message += "\nInitial Relase : June 8, 2016 (Los Angeles)\n";
        }
        if (production){
